...
# you can change this
save_folder = "embedding_savings_" + embeddings_save_file_name.split(".")[0] # remove the extension
...
def get_esm_protein(sequence):
    variant = ESMProtein(sequence=sequence)
    return variant
...
for i in tqdm(range(n_batches)):
    ...
    torch.save(embeddings, f"{save_folder}/embeddings_{i}_sequence.pt")
...
for i in range(n_batches):
    embeddings_withCoordinates.append(torch.load(f"{save_folder}/embeddings_{i}_sequence.pt"))
...
torch.save(embeddings_withCoordinates, folder_for_embeddings + "/embeddings_sequence_" + embeddings_save_file_name)
