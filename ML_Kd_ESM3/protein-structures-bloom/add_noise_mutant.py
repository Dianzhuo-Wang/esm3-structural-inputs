...
def add_noise(i):
    pdb_file = f"mutant-evoEf-6xf5/{i}/{i}_correct_mutant_no_relax.pdb"
    noises = [0.1, 0.05]
    for noise in noises:
        ...
        new_pdb.to_pdb(path=f"mutant-evoEf-6xf5/{i}/{i}_noisy_{noise}.pdb", records=["ATOM"])


if __name__ == '__main__':
    with Pool(cpu_count()) as pool:
        list(tqdm(pool.imap_unordered(add_noise, range(1, 3803+1)), total=3803))
    # add_noise(16385)
    # add_noise(16386)