...
parser.add_argument('--chain-ids', type=str, required=True,
                    help='The chain ids of the protein, ex: A,B. Mutated chain should be first. Multiple chains should be separated by a comma')
...
# Protein
protein_chains = [ProteinChain.from_pdb(protein_pdb_file, chain_id=chain_id) for chain_id in chain_ids]
protein_chains[0] = protein_chains[0].select_residue_indices(
    range(rbd_residue_index_starting_desai_sequences + index_start,
    rbd_residue_index_ending_desai_sequences + index_stop + 1))
complex_protein = ProteinChain.concat(protein_chains)
protein = ESMProtein.from_protein_chain(complex_protein)

# Assuming RBD is the first chain
index_end_rbd = np.where(complex_protein.residue_index == -1)[0][0] - 1
assert complex_protein.residue_index[0] >= rbd_residue_index_starting_desai_sequences + index_start, "The protein does not start at the right index"
assert complex_protein.residue_index[index_end_rbd] <= rbd_residue_index_ending_desai_sequences + index_stop, "The protein does not end at the right index"
...
def get_esm_protein(sequence):
    variant = ESMProtein()
    variant.sequence = sequence + complex_protein.sequence[len(sequence):]
    return variant
...
for i in tqdm(range(n_batches)):
    ...
    embeddings = embeddings.mean(dim=1)
    # embeddings_norm_layer = embeddings_norm_layer.mean(dim=1)
    torch.save(max_pooling_embeddings, f"{save_folder}/embeddings_complex_{i}_max_pooling.pt")
    torch.save(embeddings, f"{save_folder}/embeddings_complex_{i}_withCoordinates.pt")
...
for i in range(n_batches):
    embeddings_max_pooling.append(torch.load(f"{save_folder}/embeddings_complex_{i}_max_pooling.pt"))
    embeddings_withCoordinates.append(torch.load(f"{save_folder}/embeddings_complex_{i}_withCoordinates.pt"))
...
torch.save(embeddings_withCoordinates, folder_for_embeddings + "/embeddings_complex_withCoordinates_" + embeddings_save_file_name)
torch.save(embeddings_max_pooling, folder_for_embeddings + "/embeddings_complex_max_pooling_" + embeddings_save_file_name)
